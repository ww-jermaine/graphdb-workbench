    ...
    $scope.getNodes = function () {
        ...
        ClusterRestService.getClusterConfig()
            .success(function (data) {
                ...
                const leaderNode = data.find((node) => node.nodeState === 'LEADER');
                ...
                const leader = Object.values($scope.urlToNode).find((value) => value.nodeState === 'LEADER');
                Object.keys($scope.urlToNode).forEach((key) => {
                    const node = $scope.urlToNode[key];
                    const timestamp = Date.now();

                    if (node.nodeState === 'FOLLOWER') {
                        $scope.addOrUpdateLink(leader, node, 'ON', timestamp);
                    }
                });
